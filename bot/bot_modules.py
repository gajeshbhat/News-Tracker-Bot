...
def start_bot(news_bot, user_chat_session_id):
    news_bot.send_message(chat_id=user_chat_session_id.message.chat_id,
                          text="Hello! Please type /help to view commands.")


# /help command
def display_help(news_bot, user_chat_session_id):
    news_bot.send_message(chat_id=user_chat_session_id.message.chat_id, text=help_text, disable_web_page_preview=True,
...
def get_latest_news(news_bot, user_chat_session_id):
    ...
    news_bot.send_message(chat_id=user_chat_session_id.message.chat_id, text="Click on the feed",
...
def other_messages(news_bot, user_chat_session_id):
    if user_chat_session_id.message.text == "Cancel":
        ...
        news_bot.send_message(chat_id=user_chat_session_id.message.chat_id, text="Action aborted.",
                              reply_markup=reply_markup)
        clean_user_input(user_chat_session_id)
    elif user_chat_session_id.message.text == "Text Summary" and user_conversations.get(
            user_chat_session_id.message.chat_id) != {}:
        send_text_summary(user_chat_session_id)
        clean_user_input(user_chat_session_id)
    elif user_chat_session_id.message.text == "Audio Summary" and user_conversations.get(
            user_chat_session_id.message.chat_id) != {}:
        send_audio_summary(user_chat_session_id)
        clean_user_input(user_chat_session_id)
    elif user_chat_session_id.message.text == "Both" and user_conversations.get(
            user_chat_session_id.message.chat_id) != {}:
        send_text_summary(user_chat_session_id)
        send_audio_summary(user_chat_session_id)
        clean_user_input(user_chat_session_id)
        ...
        news_bot.send_message(chat_id=user_chat_session_id.message.chat_id, text="Select the format of the news.",
                              reply_markup=reply_markup)
    else:
        news_bot.send_message(chat_id=user_chat_session_id.message.chat_id,
                              text="Unknown Command! Click Cancel Button to exit from menu or type /help for instructions.")
...
def send_text_summary(user_chat_session_id):
    ...
    reply_markup = telegram.ReplyKeyboardRemove()
    news_bot.send_message(chat_id=user_chat_session_id.message.chat_id, text=text_news_summary,
                          disable_web_page_preview=True, parse_mode=telegram.ParseMode.MARKDOWN)
    news_bot.send_message(chat_id=user_chat_session_id.message.chat_id, text="Check Back later for updates.",
                          reply_markup=reply_markup)


def send_audio_summary(user_chat_session_id):
    agency_obj = News_Modules()
    news_bot.send_audio(chat_id=user_chat_session_id.message.chat_id, audio=open(
        'audio_summary/' + str(user_conversations.get(user_chat_session_id.message.chat_id)) + '-summary.mp3', 'rb'),
                        text="Audio summary.")
    reply_markup = telegram.ReplyKeyboardRemove()
    news_bot.send_message(chat_id=user_chat_session_id.message.chat_id, text="For latest summary check back later.",
                          reply_markup=reply_markup)
